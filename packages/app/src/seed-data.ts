/**
 * Seed data module for auto-populating inventory on app start.
 */

import { ItemCategory, ItemCondition, ItemStatus, PricingStrategy, SkuStatus, generateSku } from "@rent-stream/domain";
import { app } from "./api/builder.js";
import * as projection from "./api/rent-item-projection.js";
...
/**
 * Check if the inventory has already been seeded
 */
export async function checkIfSeeded(): Promise<boolean> {
  // Check if projection has any items
  return projection.itemReadModel.size > 0;
}

/**
 * Seed the inventory with realistic rental items
 */
export async function seedInventory(): Promise<void> {
  ...
  for (const item of seedItems) {
    const serialNumber = `SN-${item.name.replace(/[^a-zA-Z0-9]/g, "").substring(0, 8).toUpperCase()}-${Math.floor(Math.random() * 1000)}`;
    ...
    try {
      ...
      for (let i = 1; i <= item.quantity; i++) {
        skus.push({
          sku: generateSku(serialNumber, i),
  ...
  console.log(`\nSeed completed: ${seedItems.length} items`);
